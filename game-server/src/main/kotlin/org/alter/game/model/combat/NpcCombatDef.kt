...
data class NpcCombatDef(
    ...
    companion object {
        private const val DEFAULT_HITPOINTS = 75  // Changed from 10 to 75 (green dragon HP)
        private const val DEFAULT_ATTACK_SPEED = 4
        private const val DEFAULT_RESPAWN_DELAY = 50  // Changed from 25 to 50 (green dragon respawn)
        private const val DEFAULT_ATTACK_ANIMATION = 91  // Changed from 422 to 91 (dragon attack)
        private const val DEFAULT_BLOCK_ANIMATION = 89  // Changed from 424 to 89 (dragon block)
        private const val DEFAULT_DEATH_ANIMATION = 92  // Changed from 836 to 92 (dragon death)
        ...
        val DEFAULT =
            NpcCombatDef(
                ...
                aggressiveRadius = 10,  // Changed from 0 to 10 (green dragon aggro radius)
                aggroTargetDelay = 2,  // Changed from 0 to 2 (green dragon search delay)
                aggressiveTimer = 0,
                ...
                slayerXp = 0.0,
                bonuses = emptyList(),
                species = emptySet(),  // Note: You might want to add NpcSpecies.BASIC_DRAGON here
                LootTables = null,
                immunePoison = false,
                immuneVenom = false,
                immuneCannons = false,

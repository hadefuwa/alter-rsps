            ...
            if (!hit.cancelCondition()) {
                for (hitmark in hit.hitmarks) {
                    ...
                    if (getCurrentHp() <= 0) {
                        ...
                        setCurrentHp(0)
                        // Execute death plugin BEFORE hit actions to prevent ghost combat
                        // This ensures the entity is locked and interactions are reset before auto-retaliate can trigger
                        if (entityType.isPlayer) {
                            executePlugin(PlayerDeathAction.deathPlugin)
                        } else {
                            executePlugin(NpcDeathAction.deathPlugin)
                        }
                        // Now run hit actions after death is processed

...
abstract class KotlinPlugin(
    ...
    /**
     * A map of properties that will be copied from the [PluginMetadata] and exposed to the plugin.
     */
    private lateinit var properties: MutableMap<String, Any>

    /** Get property associated with [key] casted as [T]. */
    @Suppress("UNCHECKED_CAST") fun <T> getProperty(key: String): T? = properties[key] as? T?
    ...
    /**
     * Set the 8x8 [org.alter.game.model.region.ChunkCoords]s that belong to [region] as
     * multi-combat areas.
     */
    ...
    fun setCombatDef(
        ...
        var npc = getRSCM(npc)
        check(!r.npcCombatDefs.containsKey(npc)) {
            "Npc combat definition has been previously set: $npc"
        }
    ...
    /** Create a [ContainerKey] to register to the [World] for serialization later on. */
            ...
            when (this) {
                ...
                Direction.SOUTH -> 3
                        ...
                        throw IllegalArgumentException(
                                "Objects can only face cardinal directions. [direction=$this]"
                        )
            }
    ...
    fun npcHasOption(
        ...
        val slot =
                getNpc(getRSCM(npc)).actions.indexOfFirst { it?.lowercase() == option.lowercase() }
        return slot != -1
    }
     ...
     * This method should be used over the option-int variant whenever possible.
     *
     * @param lineOfSightDistance If the npc is behind an object such as a prison cell or bank
     * booth, this distance should be set. If the npc can be reached normally, you shouldn't specify
     * this value.
     */
    fun onNpcOption(
            npc: String,
    ...
    /**
     * Set the logic to execute when Message Class -> WindowStateMessage is handled.
     * @TODO
     */
    fun setWindowStatusLogic(logic: (Plugin).() -> Unit) = r.bindWindowStatus(logic)

    /**
     * Set the logic to execute when [net.rsprot.protocol.game.incoming.misc.user.CloseModal] is
     * handled.
     */
    fun setModalCloseLogic(logic: (Plugin).() -> Unit) = r.bindModalClose(logic)

    /**
     * Set the logic to check if a player has a menu opened and any
     * [org.alter.game.model.queue.QueueTask] with a
     * [org.alter.game.model.queue.TaskPriority.STANDARD] priority should wait before executing.
     *
     * @see PluginRepository.isMenuOpenedPlugin
     *
     * @return True if the player has a menu opened and any standard task should wait before
     * executing.
     */
    fun setMenuOpenCheck(logic: Plugin.() -> Boolean) = r.setMenuOpenedCheck(logic)

    /**
     * Set the logic to execute by default when [org.alter.game.model.entity.Pawn.attack] is
     * handled.
     */
    ...
    fun onComponentToComponentItemSwap(
            ...
            r.bindComponentToComponentItemSwap(
                    srcInterfaceId,
                    srcComponent,
                    dstInterfaceId,
                    dstComponent,
    ...
    /** Invoked when a player is sent back to their respawn location on death. */
    ...
    /**
     * Invoked when npc with [Npc.id] of [npc] finishes their death task and is de-registered from
     * the world.
     */
    fun onNpcDeath(
            npc: String,
    ...
    /** Invoked when any npc finishes their death task and is de-registered from the world. */
    ...
    /**
     * Set the combat logic for [npc] and [others], which will override the [setCombatLogic] logic.
     */
    fun onNpcCombat(
            npc: String,
            vararg others: String,
    ...
    /** Invoke [logic] when [org.alter.game.model.interf.InterfaceSet.closeByHash] is handled. */
    ...
    fun onButton(
        ...
        components.forEach { onButton(interfaceId, it, logic) }
    }
    ...
    /**
     * Invoke [logic] when an npc with [Npc.id] matching [npc] is spawned into the game with
     * [World.spawn].
     */
    fun onNpcSpawn(
            npc: String,
     ...
     * @TODO Add check if dealhit was done. -> If u have {BERSERKER_RING} and weap without overrides
     * it wont execute or just split weapons and items
     */
    ...
    /**
     * Invoke [logic] when the the option in index [option] is clicked on a [GroundItem]. String
     * option method should be used over this method whenever possible.
     */
    fun onGroundItemOption(
            item: String,
            option: Int,
    ...
    fun onSpellOnItem(
            ...
            r.bindSpellOnItem(
                    (fromInterface shl 16) or fromComponent,
                    (toInterface shl 16) or toComponent,
                    plugin
            )

    /**
     * Returns true if the item can be dropped on the floor via the 'drop' menu option - return
     * false otherwise.
     */
    fun canDropItem(
            item: String,
    ...
    /**
     * Invoke [plugin] when [itemId] (integer) is used on any NPC. This is useful for items that
     * don't have RSCM names.
     */
    ...
    fun getNpcCombatDef(npc: String): NpcCombatDef? =
            world.plugins.npcCombatDefs.getOrDefault(getRSCM(npc), null)
    ...
    /**
     * Returns how many times @param from to @param range contains. ex: @param value = 5 ex: @param
     * range = 0..90
     * @return returns 18
     *
    ...
    //        /** Run this block -> if Npc does not have specific handling for it refering to
    // ['on_item_on_npc']
    //         **/
    //    }
    //
    ...
    //        /**
    //         * Run this block -> if Object does not have specific handling for it refering to
    // ['on_item_on_obj']
    //         */
    //    }
}

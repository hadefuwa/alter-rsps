...
class KonarPlugin(
    ...
    init {
        onNpcOption("npc.konar_quo_maten", option = "talk-to") { player.queue { dialog(player) } }
    ...
    suspend fun QueueTask.dialog(player: Player) {
        ...
        when (options(player, *dialogOptions.toTypedArray())) {
            ...
            2 -> {
                ...
                chatNpc(player, "My tasks are unique - you must complete them in specific locations.")
                chatNpc(player, "You'll need at least level 75 combat to receive tasks from me.")
                chatNpc(player, "The location requirement adds difficulty but also better rewards.")
            }
            3 -> {
                chatPlayer(player, "Er, nothing actually.")
                chatNpc(player, "Return when you're ready to face location-specific challenges!")
            }
        }
    }
    ...
    suspend fun QueueTask.getAssignmentCheck(player: Player) {
        ...
        if (combatLevel < 75) {
            ...
            chatNpc(player, "Come back when you have at least level 75 combat.")
            return
        }

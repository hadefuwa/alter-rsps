 ...
 * Gate 23552 Plugin
 * 
 * This plugin handles gate 23552 (wrought iron gate), allowing players to open and close it.
 * 
 * Gate Object: gate_23552 (object 23552) - closed state
 * Opened State: gate_23554 (object 23554) - opened state
 */
class Gate23552Plugin(
    r: PluginRepository,
    world: World,
    server: Server
    ...
    companion object {
         ...
         */
        private const val CLOSED_GATE = 23552
        
        /**
         * Opened gate object ID
         * Gate 23554 is also a "wrought iron gate" and likely the opened state
         */
        private const val OPENED_GATE = 23554
    }
    ...
    init {
        ...
        // Handle gate 23552 (closed state) - open action
        ...
        try {
            if (objHasOption("object.gate_23552", "open") && !registeredOpenOptions.contains("open")) {
                onObjOption(obj = "object.gate_23552", option = "open", lineOfSightDistance = 1) {
                    openGate()
                }
                registeredOpenOptions.add("open")
            }
            
            if (objHasOption("object.gate_23552", "operate") && !registeredOpenOptions.contains("operate")) {
                onObjOption(obj = "object.gate_23552", option = "operate", lineOfSightDistance = 1) {
                    openGate()
                }
        ...
        // Handle gate 23554 (opened state) - close action
        // Track registered options to avoid duplicates
        ...
        try {
            if (objHasOption("object.gate_23554", "close") && !registeredCloseOptions.contains("close")) {
                onObjOption(obj = "object.gate_23554", option = "close", lineOfSightDistance = 1) {
                    closeGate()
                }
                registeredCloseOptions.add("close")
            }
            
            if (objHasOption("object.gate_23554", "operate") && !registeredCloseOptions.contains("operate")) {
                onObjOption(obj = "object.gate_23554", option = "operate", lineOfSightDistance = 1) {
                    closeGate()
                }

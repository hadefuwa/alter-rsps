...
class VarrockRunesShopPlugin(
    ...
    // Runes shop items - starting with 2 items, user will populate the rest
    private val storeItems = listOf(
        ShopItem(getRSCM("item.air_rune"), 1000, 10, 500),
        ShopItem(getRSCM("item.fire_rune"), 1000, 10, 500),
    )
    ...
    init {
        ...
        onNpcSpawn(shopkeeper) {
            if (npc.tile == shopkeeperTile) {
                NpcInfo(npc).setTempName("Runes Shopkeeper")
            }
        }

        // Create the runes shop with enough space for items
        createShop(
            name = "Varrock Runes Shop",
    ...
    fun Player.shop() = this.openShop("Varrock Runes Shop")

    suspend fun QueueTask.dialog(player: Player) {
        chatNpc(player, "Welcome to the Varrock Runes Shop! We have all kinds of magic runes for your spells.")

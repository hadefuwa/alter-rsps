...
open class ItemCurrency(
    ...
    override fun getSellPrice(
        ...
        val itemCost = getItem(item).cost
        // If item has no cost (unsellable items), give it a default value of 10k
        return if (itemCost == 0) {
            10000
        } else {
            Math.max(1, itemCost)
        }
    ...
    override fun getBuyPrice(
        ...
        val itemCost = getItem(item).cost
        // If item has no cost (unsellable items), give it a default value of 10k when selling to shop
        return if (itemCost == 0) {
            6000 // 60% of 10k = 6k
        } else {
            (itemCost * 0.6).toInt()

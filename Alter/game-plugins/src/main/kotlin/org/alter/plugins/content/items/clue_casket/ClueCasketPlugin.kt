...
class ClueCasketPlugin(
    ...
    private fun registerCasketHandler(itemName: String, tier: ClueTier) {
        try {
            ...
            // Fall back to registering option indices 1, 2, and 3
            ...
            for (optionIndex in 1..3) {
                try {
                    ...
                    println("DEBUG: Registered option index $optionIndex for $itemName")
                } catch (e: Exception) {
    ...
    private fun openCasket(player: Player, tier: ClueTier, itemName: String) {
        ...
        try {
            ...
            // Double-check the item ID matches
            if (item.id != casketId) {
                ...
                val removeResult = player.inventory.remove(item = casketId, amount = 1, beginSlot = slot)
                if (!removeResult.hasSucceeded()) {
                    player.message("Failed to remove casket.")
                    return
                }
